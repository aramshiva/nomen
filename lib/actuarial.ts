...
async function getMortalityData(
  gender: "M" | "F",
  ages: number[],
  ...
  const mortalityData = await db
    .select()
    .from(actuary)
    .where(and(eq(actuary.gender, gender), inArray(actuary.age, ages)))
    .execute();
...
export async function calculateSurvivalProbability(
  ...
  const nameData = await db
    .select()
    .from(names)
    .where(and(eq(names.name, name), eq(names.sex, gender)))
    .execute();
...
export async function getSurvivalProbability(
  ...
  const mortalityData = await db
    .select()
    .from(actuary)
    .where(and(eq(actuary.gender, gender), eq(actuary.age, age)))
    .execute();
